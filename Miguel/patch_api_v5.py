...
for cell in nb["cells"]:
    ...
    if "%%writefile api_aemet.py\n" in cell["source"][0] or "%%writefile api_aemet.py" in cell["source"][0]:
        ...
        for i, line in enumerate(source):
            if "SELECT fecha, tmed" in line:
                source[i] = line.replace("SELECT fecha, tmed", "SELECT fecha, tmed, nombre")
            elif "return {\"fecha\": str(fila[0]), \"tmed\": float(fila[1])}" in line:
    ...
    if "%%writefile web_aemet.py\n" in cell["source"][0] or "%%writefile web_aemet.py" in cell["source"][0]:
        source = cell["source"]
        for i, line in enumerate(source):
            if "st.success(f\"Última temperatura media registrada: {valor} °C (Fecha: {fecha_ult}) — IDEMA: {idema}\")" in line:
                source[i] = line.replace(
